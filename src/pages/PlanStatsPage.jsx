...
import { FaArrowLeft, FaChartLine, FaCalendarAlt, FaDumbbell, FaWeight, FaStopwatch, FaChartPie, FaChartBar } from 'react-icons/fa';
...
import BasicStatsCard from '../components/analytics/BasicStatsCard';
import VolumeStatsCard from '../components/analytics/VolumeStatsCard';
import IntensityStatsCard from '../components/analytics/IntensityStatsCard';
import ProgressionStatsCard from '../components/analytics/ProgressionStatsCard';
import ComplianceStatsCard from '../components/analytics/ComplianceStatsCard';
import ExerciseDistributionCard from '../components/analytics/ExerciseDistributionCard';
...
const PlanStatsPage = () => {
  ...
  useEffect(() => {
    if (trainingPlans && planId && workoutLogs) {
      ...
      if (foundPlan) {
        ...
        const advanced = analyticsService.analyzePlan(foundPlan, filteredLogs);
  ...
  const calculateStats = (logs) => {
    if (!logs || logs.length === 0 || !plan) {
      return;
    }

    // Usar el servicio de estadísticas para calcular las estadísticas del plan
    const calculatedStats = statisticsService.calculatePlanStats(plan, { logs });
  ...
  return (
    <div className="container mx-auto px-4 py-8 pt-16 pb-24 max-w-lg">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md overflow-hidden mb-6">
        ...
        <div className="p-4 bg-gray-50 dark:bg-gray-700 border-b border-gray-200 dark:border-gray-600 flex justify-center">
          <div className="inline-flex rounded-md shadow-sm" role="group">
            <button
              type="button"
              onClick={() => setViewMode('basic')}
              className={`px-4 py-2 text-sm font-medium rounded-l-lg ${viewMode === 'basic'
            ...
            <button
              ...
              className={`px-4 py-2 text-sm font-medium rounded-r-lg ${viewMode === 'advanced'
                ? 'bg-primary-600 text-white'
                : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700'}`}
            >
      ...
      {/* Estadísticas avanzadas */}
      {viewMode === 'advanced' && advancedStats && (
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <BasicStatsCard stats={advancedStats.basic} />
            <VolumeStatsCard stats={advancedStats.volume} />
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <IntensityStatsCard stats={advancedStats.intensity} />
            <ProgressionStatsCard stats={advancedStats.progression} />
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <ComplianceStatsCard stats={advancedStats.compliance} />
            <ExerciseDistributionCard stats={advancedStats.exerciseDistribution} />
          </div>
        </div>
      )}
          ...
          {stats.totalWorkouts > 0 && (
            <div className="space-y-6">
              <div>
                ...
                <MuscleGroupDistribution stats={stats} />
              </div>
          ...
          {stats.exerciseStats && Object.keys(stats.exerciseStats).length > 0 ? (
            <div className="space-y-6">
              {Object.values(stats.exerciseStats)
                ...
                .map(exercise => {
                  // Obtener datos de progreso del ejercicio
                  const exerciseProgress = statisticsService.calculateExerciseProgress(exercise.name, { logs: planLogs });

                  // Preparar datos para el gráfico de progreso
                  const progressData = exerciseProgress?.progressData.map(point => ({
                    label: formatDate(point.date),
                    value: point.maxWeight
                  })) || [];

                  const maxValue = progressData.length > 0 ?
                    Math.max(...progressData.map(item => item.value)) * 1.2 : 0;

                  // Calcular tendencia
                  let trendText = '';
                  let trendColor = '';

                  if (exerciseProgress && exerciseProgress.weightTrend !== 0) {
                    const trend = exerciseProgress.weightTrend;
                    if (trend > 0) {
                    ...
                    <div key={exercise.name} className="border-b border-gray-200 dark:border-gray-700 pb-6 last:border-0 last:pb-0">
                      ...
                      <div className="bg-gray-50 dark:bg-gray-700 p-3 rounded-lg">
                        ...
                        {progressData.length > 0 ? (
                          ...
                          <p className="text-xs text-gray-500 dark:text-gray-400 text-center py-2">No hay suficientes datos para mostrar el progreso</p>
                        )}
                      </div>
                    </div>
                  );
