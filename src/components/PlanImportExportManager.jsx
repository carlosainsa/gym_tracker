...
import { FaFileExport, FaFileImport, FaDownload, FaUpload, FaCheck, FaTimes, FaInfoCircle, FaFileCsv, FaFileCode } from 'react-icons/fa';
...
const PlanImportExportManager = () => {
  ...
  // Manejar la exportación de planes seleccionados a JSON
  const handleExportSelectedPlansToJson = () => {
    try {
      ...
      setImportStatus({
        success: true,
        message: `${plansToExport.length} ${plansToExport.length === 1 ? 'plan exportado' : 'planes exportados'} correctamente como JSON`
      });
    } catch (error) {
      console.error('Error al exportar planes a JSON:', error);
      setImportStatus({
        success: false,
        message: 'Error al exportar planes a JSON: ' + error.message
      });
    }
  };

  // Manejar la exportación de planes seleccionados a CSV
  const handleExportSelectedPlansToCsv = () => {
    try {
      if (selectedPlans.length === 0) {
        setImportStatus({
          success: false,
      ...
      if (selectedPlans.length > 1) {
        setImportStatus({
          success: false,
          message: 'La exportación a CSV solo permite un plan a la vez'
        });
        return;
      }

      const planToExport = trainingPlans.find(plan => plan.id === selectedPlans[0]);
      importExportService.downloadPlanAsCsv(planToExport);

      setImportStatus({
        success: true,
        message: `Plan "${planToExport.name}" exportado correctamente como CSV`
      });
    } catch (error) {
      console.error('Error al exportar plan a CSV:', error);
      setImportStatus({
        success: false,
        message: 'Error al exportar plan a CSV: ' + error.message
      });
    }
  };

  // Manejar la exportación de todos los planes a JSON
  const handleExportAllPlansToJson = () => {
    try {
      importExportService.downloadAllPlansAsJson(trainingPlans);

      setImportStatus({
        success: true,
        message: `${trainingPlans.length} ${trainingPlans.length === 1 ? 'plan exportado' : 'planes exportados'} correctamente como JSON`
      });
    } catch (error) {
      console.error('Error al exportar todos los planes a JSON:', error);
      setImportStatus({
        success: false,
        message: 'Error al exportar todos los planes a JSON: ' + error.message
      });
    }
  };
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      ...
      if (isCSV) {
        // Importar desde CSV
        const planName = file.name.replace(/\.csv$/i, '').replace(/_/g, ' ');
        const importedPlan = importExportService.importPlanFromCsv(text, {
          planName,
          planDescription: `Plan importado desde CSV: ${file.name}`
        });

        // Agregar el plan importado
        importedPlans = importPlanFromJson(JSON.stringify(importedPlan));
      } else {
        ...
        // Importar desde JSON
        importedPlans = importPlanFromJson(text);
      }
      ...
      <div className="p-6">
        ...
        <div className="mb-8">
          ...
          <div className="flex flex-wrap gap-3">
            <button
              onClick={handleExportAllPlansToJson}
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-2">
              <button
                onClick={handleExportSelectedPlansToJson}
              ...
              >
                <FaFileCode className="mr-1.5" />
                JSON
              </button>

              <button
                onClick={handleExportSelectedPlansToCsv}
                ...
                title="Exportar como CSV (solo un plan a la vez)"
              >
        ...
        <div className="mt-8 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
          ...
          <ul className="text-sm text-blue-700 dark:text-blue-400 space-y-1 ml-6 list-disc">
            <li>Los planes se pueden exportar en formato JSON o CSV</li>
            <li>El formato JSON conserva toda la información del plan y es ideal para copias de seguridad</li>
